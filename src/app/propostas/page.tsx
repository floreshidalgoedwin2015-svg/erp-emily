...
export default async function PropostasPage({
  ...
  let query = supabase
    .from("propostas")
    .select("id, status, total, cliente_nome, validade, criado_em")
    .order("criado_em", { ascending: false });

  if (filtroAtual !== "todos") query = query.eq("status", filtroAtual);
    ...
    <div className="p-8 max-w-5xl">
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-zinc-800">📋 Propostas comerciais</h1>
            <p className="text-zinc-500 text-sm mt-0.5">{total} proposta{total !== 1 ? "s" : ""}</p>
          </div>
          <a href="/propostas/nova" className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-600 text-white text-sm font-semibold hover:bg-amber-700 transition shrink-0">
            + Nova proposta
          </a>
        </div>
      </div>
      ...
      <div className="flex gap-1.5 flex-wrap mb-4">
        {FILTROS.map((f) => (
          <a key={f.value} href={f.value === "todos" ? "/propostas" : `/propostas?status=${f.value}`}
      ...
      {total === 0 ? (
        <div className="bg-white rounded-2xl border border-amber-100 p-12 text-center shadow-sm">
          ...
          <h3 className="text-lg font-semibold text-zinc-800 mb-2">Nenhuma proposta ainda</h3>
          <p className="text-zinc-500 text-sm mb-6">Crie propostas comerciais para enviar às clientes.</p>
          <a href="/propostas/nova" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-600 text-white text-sm font-semibold hover:bg-amber-700 transition">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-amber-100 bg-amber-50">
                <th className="text-left px-5 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wide">#</th>
                ...
                <th className="text-center px-5 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wide hidden md:table-cell">Validade</th>
              ...
              {propostas?.map((p) => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-amber-50/40 transition-colors">
                    ...
                    <td className={`px-5 py-3 text-center text-xs hidden md:table-cell ${vencida ? "text-red-500 font-semibold" : "text-zinc-500"}`}>
                      {p.validade
                        ? new Date(p.validade + "T12:00:00").toLocaleDateString("pt-BR")
                        : "—"}
                    </td>
                    <td className="px-5 py-3 text-right">
                      <a href={`/propostas/${p.id}`} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border border-amber-200 text-amber-700 text-xs font-semibold hover:bg-amber-50 hover:border-amber-300 transition">
                        Ver
                      </a>
                    </td>
                  </tr>
