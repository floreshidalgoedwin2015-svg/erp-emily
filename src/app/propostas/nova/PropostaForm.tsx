...
type ProdutoOpcao = {
  id: number;
  nome: string;
  variacoes: { id: number; tamanho: string | null; cor: string | null; preco_venda: number }[];
};
...
export function PropostaForm({ produtos }: { produtos: ProdutoOpcao[] }) {
  ...
  function adicionarVariacao(produto: ProdutoOpcao, v: { id: number; tamanho: string | null; cor: string | null; preco_venda: number }) {
  ...
  function atualizarItem(key: string, campo: "quantidade" | "preco_unitario", valor: number) {
    setItens((prev) =>
      prev.map((i) => {
        if (i.key !== key) return i;
        const novo = { ...i, [campo]: valor };
        novo.subtotal = novo.quantidade * novo.preco_unitario;
        return novo;
      }),
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="bg-white rounded-2xl border border-amber-100 p-6 shadow-sm">
        <h2 className="text-sm font-bold text-zinc-700 uppercase tracking-wide mb-4">Dados da proposta</h2>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <input
              ...
              placeholder="Condições especiais, prazo de entrega..."
              className={inp}
            />
          </div>
        </div>
      ...
      {/* Produtos */}
      <section className="bg-white rounded-2xl border border-amber-100 p-6 shadow-sm">
        <h2 className="text-sm font-bold text-zinc-700 uppercase tracking-wide mb-4">Produtos da proposta</h2>
            ...
            <div className="flex flex-wrap gap-2">
              {produtoSelecionado.variacoes.map((v) => (
                <button key={v.id} type="button" onClick={() => adicionarVariacao(produtoSelecionado, v)}
                  ...
                  {descVariacao(v)} <span className="text-xs text-zinc-400 ml-1">{fmt(v.preco_venda)}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {itens.length === 0 ? (
          ...
          <>
            <div className="hidden sm:grid sm:grid-cols-[2fr_1fr_1fr_1fr_auto] gap-2 px-2 mb-1">
              ...
              <span className="text-xs font-semibold text-zinc-400 uppercase">Preço unit.</span>
            ...
            <div className="mt-4 flex justify-end">
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-6 py-3 text-right">
                <div className="text-xs text-zinc-500">Total da proposta</div>
      ...
      <div className="flex gap-3 justify-end pb-8">
        <a href="/propostas" className="px-5 py-2.5 rounded-xl border border-zinc-200 text-sm font-medium text-zinc-600 hover:bg-zinc-50 transition">Cancelar</a>
