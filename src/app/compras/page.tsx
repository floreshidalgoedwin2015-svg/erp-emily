...
export default async function ComprasPage({
  ...
  let query = supabase
    .from("pedidos_compra")
    .select("id, status, total, data_prevista, criado_em, fornecedores ( nome )")
    ...
    <div className="p-8 max-w-5xl">
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-zinc-800">🛍️ Pedidos de compra</h1>
            <p className="text-zinc-500 text-sm mt-0.5">{total} pedido{total !== 1 ? "s" : ""}</p>
          </div>
          <Link href="/compras/novo" className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-600 text-white text-sm font-semibold hover:bg-amber-700 transition shrink-0">
            + Novo pedido
          </Link>
        </div>
      </div>
      ...
      <div className="flex gap-1.5 flex-wrap mb-4">
        {FILTROS.map((f) => (
          <a key={f.value} href={f.value === "todos" ? "/compras" : `/compras?status=${f.value}`}
      ...
      {total === 0 ? (
        <div className="bg-white rounded-2xl border border-amber-100 p-12 text-center shadow-sm">
          ...
          <p className="text-zinc-500 text-sm mb-6">Crie um pedido para registrar compras de fornecedores.</p>
          <Link href="/compras/novo" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-600 text-white text-sm font-semibold hover:bg-amber-700 transition">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-amber-100 bg-amber-50">
                ...
                <th className="text-left px-5 py-3 text-xs font-semibold text-zinc-500 uppercase tracking-wide hidden sm:table-cell">Fornecedor</th>
            ...
            <tbody className="divide-y divide-zinc-100">
              {pedidos?.map((p) => {
                const fornecedor = p.fornecedores as unknown as { nome: string } | null;
                return (
                  <tr key={p.id} className="hover:bg-amber-50/40 transition-colors">
                    <td className="px-5 py-3"><span className="font-mono font-semibold text-amber-700">#{p.id}</span></td>
                    <td className="px-5 py-3 hidden sm:table-cell"><span className="text-zinc-700 font-medium">{fornecedor?.nome ?? "—"}</span></td>
                    ...
                    <td className="px-5 py-3 text-right">
                      <Link href={`/compras/${p.id}`} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border border-amber-200 text-amber-700 text-xs font-semibold hover:bg-amber-50 hover:border-amber-300 transition">
                        Ver
                      </Link>
                    </td>
                  </tr>
